...
## Answer 
> Yes, despite a dip in the mid 1600s, there appears to be a moderately positive trend of the number of girls baptized over time. We could say that there appears to be a moderately positive correlation between the number of girls baptized and the number of years elapsed since 1629, despite an abbreation in this trend during the period of roughly 1640-1660, which might be worth exploring further.  
...
```{r plot-proportion_of_boys_over_time, eval=TRUE}
plot(arbuthnot$year, arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), type = "l")
```

### Answer 
> The proportion data appears much noiser than the count data. It's possible there is a slight negative trend in the average propotion of boys over time, but given the noiseness of the data, I would want to look more closely before drawing any conclusions. I played around with trying to plot some sort of mean line through the graph to make a potential trend more obvious but wasn't quite successful yet. 
...
> There are 63 records (rows) with 3 attributes (columns). The attributres are year, boys, and girls. Boys and girls look like count totals. The data runs from 1941 to 2002. 

#### Code
``` {r Question_1, eval=TRUE}
dim(present)
head(present)
...
> The counts themselves are much higher and are not on a similar scale to Arbuthnot's. We could potentially plot proportions on a more similar scale. 
...
```{r ratio_boys_to_girls, eval=TRUE}
plot(present$year, present$boys / present$girls, type = "l", xlab = 'Year', ylab = 'Ratio of boys to girls')
```

#### Answer

> The above plot suggests that Arbuthnot's observation still holds true. The ratio of boys to girls born appears to suggest boys are still born at a slightly greater rate, even as this trend has declined over the past sixty years of data. 

-   In what year did we see the most total number of births in the U.S.? 

#### Code 
```{r max_total_births_year, eval=TRUE}
present$total <- present$boys + present$girl
...
>In 1961. 
I decide to create a column by assignment that was the total number of boys and girls. Which.max was a helpful command from the suggested reference card to return the index of the row with the max for the variable. I call the year attribute for that index and end up with 1961. I used present total for a quick spot check. 
